...
**Create a complete OpenNebula marketplace appliance from a Docker container**

## 📋 Overview

This tutorial shows you how to create an OpenNebula appliance that:
- ✅ Runs a Docker container automatically on VM startup
- ✅ Has SSH access with key authentication from OpenNebula
...
An **OpenNebula appliance** consists of:

1. **QCOW2 VM Image** - The bootable virtual machine disk image
2. **Metadata Files** - YAML files describing the appliance
3. **VM Template** - OpenNebula template for instantiation
...
## 📝 Step 2: Create Appliance Files

### 2.1 Create Appliance Directory

```bash
# Create directory for your appliance (use lowercase, no spaces)
...
Create `metadata.yaml` with your appliance information:
