...
APPLIANCE_NAME="nextcloud-aio-aarch64"
...
SERIAL_EOF
    ...
    # ARM64 (the QEMU "virt" machine) exposes its primary UART as ttyAMA0 (PL011),
    # not ttyS0. Provide an autologin getty there so the OpenNebula serial console
    # and `virsh console` work on aarch64 guests.
    if [ "$(uname -m)" = "aarch64" ]; then
        mkdir -p /etc/systemd/system/serial-getty@ttyAMA0.service.d
        cat > /etc/systemd/system/serial-getty@ttyAMA0.service.d/override.conf << 'SERIAL_AMA_EOF'
[Service]
ExecStart=
ExecStart=-/sbin/agetty --noissue --autologin root %I 115200,38400,9600 vt102
Type=idle
